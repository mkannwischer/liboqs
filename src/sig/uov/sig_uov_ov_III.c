...
OQS_SIG *OQS_SIG_uov_ov_III_new(void) {
	...
	sig->length_secret_key = OQS_SIG_uov_ov_III_length_secret_key;
	sig->length_signature = OQS_SIG_uov_ov_III_length_signature;

	sig->keypair = OQS_SIG_uov_ov_III_keypair;
	sig->sign = OQS_SIG_uov_ov_III_sign;
	sig->verify = OQS_SIG_uov_ov_III_verify;
	sig->sign_with_ctx_str = OQS_SIG_uov_ov_III_sign_with_ctx_str;
	sig->verify_with_ctx_str = OQS_SIG_uov_ov_III_verify_with_ctx_str;

	return sig;
}

extern int pqov_uov_III_ref_keypair(uint8_t *pk, uint8_t *sk);
...
OQS_API OQS_STATUS OQS_SIG_uov_ov_III_sign_with_ctx_str(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *secret_key) {
	if (ctx_str == NULL && ctx_str_len == 0) {
		return OQS_SIG_uov_ov_III_sign(signature, signature_len, message, message_len, secret_key);
	} else {
		return OQS_ERROR;
	}
}

OQS_API OQS_STATUS OQS_SIG_uov_ov_III_verify_with_ctx_str(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *public_key) {
	if (ctx_str == NULL && ctx_str_len == 0) {
		return OQS_SIG_uov_ov_III_verify(message, message_len, signature, signature_len, public_key);
	} else {
		return OQS_ERROR;
	}
}
